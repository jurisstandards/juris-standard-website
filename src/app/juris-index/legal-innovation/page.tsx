...
export default function LegalInnovationExcellenceTerminal() {
  ...
  useEffect(() => {
    async function fetchRecords() {
      try {
        ...
        // Filter out inactive records and only keep Legal Innovation Excellence division
        const relevant = data.filter((d: any) => d.status === "active" && d.division.startsWith("Legal Innovation Excellence"));
  ...
  const bands = [
      { title: "Innovation Excellence", tag: "THE PRINCIPAL RECORD", firms: filteredFirms.filter(f => f.tier === "01" || f.category === "Innovation Excellence") },
      { title: "Legal Research", tag: "DISTINGUISHED RECORD", firms: filteredFirms.filter(f => f.tier === "02" || f.category === "Legal Research") },
      { title: "Legal Technology", tag: "NEXT GENERATION", firms: filteredFirms.filter(f => f.tier === "03" || f.category === "Legal Technology") },
    ];
                  ...
                  <div className="flex items-stretch gap-3 min-w-max">
                    {band.firms.length > 0 ? band.firms.map((firm, fIdx) => (
                      <div
                        key={fIdx}
                        onClick={() => {
                          router.push(`/juris-index/legal-innovation/${firm.id}`);
                        }}
