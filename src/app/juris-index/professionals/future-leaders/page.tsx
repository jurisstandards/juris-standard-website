  ...
  return (
    <main className="min-h-screen bg-[#000000] selection:bg-[#CBAA69]/30 flex flex-col font-sans text-[#FFFFF0] overflow-x-hidden">
          ...
          <div className="w-full lg:w-[380px] shrink-0 border border-white/10 bg-[#0a0a0a]/95 backdrop-blur-3xl rounded-[2px] p-7 shadow-[0_30px_60px_rgba(0,0,0,0.8)] relative z-20">
            ...
            <div className="flex flex-col gap-5 mb-8">
              {[
                ...
                { label: 'RECOGNITION', val: 'Future Leaders™ - 2027', icon: Award, options: ['2027','2026','2025'], state: recognition, set: setRecognition }
              ].map((dropdown, idx) => (
      ...
      <div ref={lawyersRef} className="flex flex-col w-full relative z-10 bg-[#000000]">
        {[
          { tag: "THE PRINCIPAL RECORD", title: "Future Leaders 2027", desc: "", lawyers: [] as any[] },
          { tag: "DISTINGUISHED RECORD", title: "Emerging Leaders", desc: "", lawyers: [] as any[] },
          { tag: "NEXT GENERATION", title: "Rising Stars", desc: "", lawyers: [] as any[] }
        ].map((band, idx) => {
                ...
                <div className="overflow-x-auto pb-4 pt-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                  <div className="flex items-stretch gap-4 min-w-max pr-12">
                    {mergedLawyers.map((lawyer, fIdx) => (
                      <Link key={fIdx} href={`/juris-index/professionals/future-leaders/${(lawyer as any).id || lawyer.name.toLowerCase().replaceAll(' ', '-')}`} className="w-[200px] sm:w-[220px] h-[300px] flex flex-col border border-[#1a1a1a] bg-[#080808] hover:border-[#CBAA69]/40 transition-colors duration-200 relative cursor-pointer group/card overflow-hidden">
