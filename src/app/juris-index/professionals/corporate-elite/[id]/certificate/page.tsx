...
export default function CorporateEliteCertificatePage() {
  ...
  const currentDate = record.created_at ? new Date(record.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) : "16 September 2026";
  const roleName = record.firmInfo?.designation || record.type || 'CORPORATE & M&A COUNSEL™';
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: `The Juris Standard - ${record?.name}`,
          ...
          <div className="w-full h-full relative overflow-hidden rounded-sm shadow-inner">
            ...
            <div className="w-full h-full relative z-10 flex">
                ...
                <div className="relative z-10 flex-1 flex flex-col justify-between w-full">
                  <div className="flex-1 flex items-center justify-center mt-10">
                     <p className="text-[#CBAA69]/60 tracking-[0.25em] text-[0.55rem] uppercase leading-[2.5] text-center font-medium w-full px-4 break-words">
                       EXCELLENCE RECOGNISED.<br/><br/>A STRONGER<br/>LEGAL WORLD.<br/>ALWAYS.
                     </p>
                  </div>
              ...
              <div className="w-[78%] h-full relative flex flex-col items-center py-10 px-12 text-center text-[#111]">
                ...
                <p className="text-[0.6rem] tracking-[0.1em] text-[#111] uppercase font-bold mb-6">
                  {roleName}
                </p>
                ...
                <div className="w-full flex justify-between items-end mt-4 relative pt-12 px-4">
                  ...
                  <div className="flex flex-col items-start gap-2 pb-4">
                    <div className="font-[Brush_Script_MT,cursive] text-[2.4rem] text-[#0a0a0a] opacity-90 -mb-2 pl-2">Gabriel Jane</div>
                ...
                <div className="absolute top-10 right-10 flex flex-col items-end gap-3 text-right opacity-70">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[0.6rem] tracking-widest text-[#111] font-bold">{recognitionId}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[0.45rem] tracking-[0.2em] text-[#444] uppercase font-bold">Status</span>
                    <span className="text-[0.6rem] tracking-widest text-[#111] font-bold">{record.status || 'Active'}</span>
                  </div>
                </div>
