...
import { 
  ...
  Briefcase, AtSign, Camera, Phone, Mail, MoreHorizontal, Search, User
} from "lucide-react";
...
export default function CorporateEliteShareCardPage() {
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-[#0a0a0a]">
          ...
          <div className="flex-1 flex flex-col p-6 lg:p-8 border-b border-[#222] relative min-h-0 items-center justify-center">
            ...
            <div className="flex-1 flex items-center justify-center w-full relative">
              ...
              <div 
                className="w-full max-h-full flex relative overflow-hidden bg-[#111] shadow-2xl border border-white/5 group"
              ...
              >
                ...
                <img 
                  ...
                  alt="Background" 
                  className="absolute inset-0 w-full h-full object-cover opacity-60 mix-blend-lighten sepia-[0.3]"
                  style={{ objectPosition: '70% center' }}
                />
                
                {/* Right side Portrait/Image */}
                <div className="absolute right-[5%] bottom-0 w-[55%] h-[90%] flex items-end justify-center overflow-hidden z-10 pointer-events-none">
                  {/* Mock user silhouette */}
                  <div className="w-full h-full bg-gradient-to-t from-black via-black/80 to-transparent absolute inset-0 z-20" />
                  <User className="w-[80%] h-[120%] text-white/10 mb-[-20%] relative z-10" strokeWidth={1} />
                </div>

                <div className="absolute inset-0 bg-gradient-to-r from-black via-black/90 to-transparent w-[60%] z-10" />
                ...
                <div className="relative z-20 w-full h-full flex flex-col justify-between p-6 sm:p-10 md:p-14 lg:p-16">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12">
                      <img src="/logo/seal main.png" className="w-full h-full object-contain sepia-[0.3]" />
                    </div>
                    <div className="flex flex-col">
                      <h4 className="font-serif text-[0.8rem] lg:text-[1rem] text-white tracking-[0.25em] uppercase mb-0.5">
                        The Juris Standard
                      </h4>
                      <span className="text-[0.35rem] lg:text-[0.4rem] text-white/60 tracking-[0.3em] uppercase">
                  ...
                  <div className="flex-1 flex flex-col justify-center max-w-[60%]">
                    <h5 className="font-serif text-[0.6rem] lg:text-[0.8rem] text-white/90 tracking-[0.2em] uppercase mb-2 font-light">
                      RECOGNISED · {record.year || '2027'}
                    </h5>
                    
                    <h2 className="font-serif text-[2.5rem] lg:text-[4rem] text-white uppercase tracking-wider mb-8 font-medium leading-none drop-shadow-lg">
                      CORPORATE<br/>ELITE™
                    </h2>

                    <h3 className="font-serif text-xl lg:text-3xl text-white uppercase tracking-wider mb-2">
                      {record.name}
                    </h3>
                    <p className="font-serif text-sm lg:text-lg text-white/80 mb-6">
                      Corporate & M&A Counsel™
                    </p>

                    <div className="flex flex-col text-[0.55rem] lg:text-[0.65rem] text-white/60 uppercase tracking-widest gap-1">
                      <span>{record.firmInfo?.designation || record.type || 'Partner'}</span>
                      <span className="text-white">{record.firmInfo?.firm_name || record.name}</span>
                      <span>{record.location}, {record.jurisdiction || 'India'}</span>
                    </div>
                  </div>

                  {/* Bottom Left */}
                  <div className="flex items-center gap-4 text-[0.35rem] lg:text-[0.45rem] tracking-[0.25em] uppercase text-white/40 border-t border-white/20 pt-4 w-max pr-12">
                    <span>PEOPLE</span>
                    <span>FIRMS</span>
                    <span>IDEAS</span>
                    <span>IMPACT</span>
                ...
                {/* Right Elements (QR, Vertical Text) */}
                <div className="absolute top-0 right-0 bottom-0 w-[40%] flex flex-col items-end justify-between p-6 sm:p-10 md:p-12 z-20 pointer-events-none">
                  ...
                  <div className="flex flex-col items-center bg-black/40 backdrop-blur-md p-3 border border-white/10 rounded-sm">
                    <div className="w-12 h-12 bg-white p-1 mb-2">
                    ...
                    <span className="text-[0.35rem] tracking-[0.2em] uppercase text-white/80 text-center leading-[1.4] w-20">
                      VERIFY AT THE JURIS STANDARD™
                    </span>
                  </div>
                </div>
            ...
            <div className="flex-1 flex items-stretch gap-5 overflow-x-auto pb-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              {FORMATS.filter(f => f.id !== selectedFormat && f.id !== 'custom').map(format => (
                <div key={format.id} className="flex flex-col h-full shrink-0 group cursor-pointer" onClick={() => setSelectedFormat(format.id)}>
                  ...
                  >
                    <img src="/collections/corporate_elite_bg.png" className="absolute inset-0 w-full h-full object-cover sepia-[0.3]" />
                    <div className="absolute inset-0 bg-gradient-to-r from-black via-black/80 to-transparent w-[80%]" />
                    <div className="absolute right-0 bottom-0 w-[50%] h-[80%] flex items-end justify-center opacity-30">
                      <User className="w-full h-[120%] mb-[-10%]" strokeWidth={1} />
                    </div>
                    <div className="absolute inset-0 p-3 flex flex-col justify-between z-10">
                      <div className="flex flex-col h-full justify-center">
                        <h4 className="font-serif text-[0.35rem] text-white/70 uppercase mb-0.5">RECOGNISED · 2027</h4>
                        <h2 className="font-serif text-[0.65rem] text-white uppercase font-medium leading-none mb-1">Corporate Elite™</h2>
                        <span className="font-serif text-[0.45rem] text-white/90">{record.name}</span>
                      </div>
                    </div>
                  </div>
                </div>
